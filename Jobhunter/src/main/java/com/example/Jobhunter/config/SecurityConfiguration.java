...
public class SecurityConfiguration {
    ...
    private SecretKey getSecretKey() {
        ...
        return new SecretKeySpec(keyBytes, 0, keyBytes.length, SecurityUtil.JWT_Algorithm.getName()); //gói mảng byte thành một đối tượng SecretKey, chỉ định thuật toán (HS512).
    }

// Đoạn code này để tạo và cấu hình một JwtEncoder (máy tạo token). Nó sẽ dùng secret key bạn cấu hình để ký JWT.
// Sau này trong hàm createToken(Authentication authentication), bạn gọi jwtEncoder.encode(...) → token sẽ sinh ra.
}